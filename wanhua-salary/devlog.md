...
### 做工具之前，最關鍵的一步：先把規則徹底搞懂

這是整個專案我認為**最重要、也最容易被跳過**的一步 —— 而且它跟寫 code 完全無關。

要能做薪資結算，我得先**徹底了解整個團隊在「算薪資」這件事上的所有流程**：

1. **訪談各方** — 店長怎麼看打卡？會計怎麼對帳？哪些數字是手動喬的？哪些規則只存在某個人腦袋裡？
2. **蒐集各方資料、了解痛點** — 把散落在 Excel 公式、LINE 訊息、口頭慣例裡的計算邏輯全部攤開。
3. **整理成一份「員工薪資計算規則」文件**（[公開版 v1.2](https://wanhua-dashboard.vercel.app/docs/salary-rules-draft.html)）—— 辛苦金、業績獎金、Winnie 店長業績、國定假日、加班費、勞健保自付額、特殊調整的使用時機，每一條都白紙黑字寫下來、跟團隊對齊。

**有了這份規則文件，才有辦法做這個工具。** 規則文件不是「文件工作」，它是這個工具的規格書、是計算邏輯的單一真相來源、也是跟團隊溝通的介質。後面所有的 Python 計算、UI、防呆，全部是這份文件的延伸。

> 📄 **這份地基文件（公開可讀）**：[員工薪資計算規則 v1.2 →](https://wanhua-dashboard.vercel.app/docs/salary-rules-draft.html)
> 想了解這個工具怎麼把規則固化下來，直接看這份文件最快。

> 順序很關鍵：**先把規則搞懂並寫下來，才開始做工具**。反過來（邊寫 code 邊問規則）會永遠對不齊。
...
**Phase 1：把規則徹底搞懂、寫成文件**（2 天，但最關鍵）
不是「翻譯試算表公式」這麼簡單 —— 是**訪談店長/會計、蒐集各方散落的計算邏輯與痛點**，再請 AI 幫我整理成一份完整的自然語言規則文件（`docs/salary-rules-draft.html`），跟團隊逐條對齊。這一步的目的是**把隱性規則顯性化、變成單一可信來源**，完全不是寫 code。對完之後才有信心開工 —— 沒有這份文件，後面什麼都做不了。
...
最值得分享的不是技術細節，是**「做薪資工具的前提，是先徹底搞懂全團隊的薪資計算流程、整理成一份規則文件」這件事**。工具只是規則的載體；沒先把規則搞懂，工具根本無從做起。

我的順序是：

1. **訪談各方** — 店長、會計，了解整個算薪資的流程怎麼跑
2. **蒐集資料 + 了解痛點** — 把 Excel 公式、LINE 慣例、口頭規則全攤開
3. **整理成規則文件** — 跟團隊逐條對齊，變成單一可信來源
4. **才開始做工具** — 對齊之後，Python 計算是相對機械的翻譯動作

絕大多數人的順序是反的：先動手寫 code → 邊寫邊發現規則沒講清 → 改 code → 再改規則 → 永遠不一致。後來每次業務規則調整（門檻、4h、Winnie 算法、會計工作流），也都是先改文件 → 改設定 → 改 code，三邊同步。
