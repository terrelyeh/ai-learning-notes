...
| 分頁 | 解了什麼痛點 | 自動化亮點 |
|---|---|---|
| **儀表板** | 「現在狀況怎樣？」要一直 query 才知道 | KPI（訂單數 / 預估營業額 / 總折扣 / 待對帳人數）+ 上限剩餘量 + 智能 Todos |
| **訂單明細** | 客人資料散在 36 欄要橫向掃 | 一筆一列、總額計算好、地址 / 電話 / 後五碼 / 異常並排可看 |
| **品項採購總** | 每個品項要逐欄 SUM、且要分自取/宅配 | 自動分「累計 / 自取 / 宅配 / 其他」4 欄 — 直接給廠商叫貨用 |
| **通路分析** | 沒人有時間做交叉分析 | 4 個通路自動分組 + 訂單數 / 金額 / 平均單價 / 收款狀態 + 兩個甜甜圈圖（金額佔比 + 訂單數佔比）+ 各通路客戶 drill-down |
| **匯款對帳** | LINE 上找後五碼跟 Sheet cross-check 容易漏 | 自動依後五碼分類「已對帳 / 待對帳」 + 待對帳排前面 + 統計列 |
| **異常清單** | 出貨後才發現問題 | 即時把有疑問的訂單獨立列出來，紅底警示 |
