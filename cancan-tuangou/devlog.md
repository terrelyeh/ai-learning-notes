...
> 🤖 **我用 AI 做了什麼**：把每檔次團購訂單整理的痛苦從「下午 4 小時手動」變成「全自動」，加上每筆訂單一鍵生成的設計版宅配出貨單 PDF，以及手機即時收到的 Telegram 訂單通知。
> ⏱ **沒有 AI 的話**：找工程師外包估計 3-6 萬，外加每次有修改要 ticket。自己做？我連 Apps Script 是什麼都不太確定。
> ✅ **最終成果**：一份 Google Sheet 上自動跑 6 個分析分頁、Telegram + email 雙管道通知、設計版宅配 PDF（用 Apps Script Web App + HTML 模板，瀏覽器列印零月費）。
...
- **debug 夥伴**：9 個坑都是 AI 跟我一起 trace 出來的（從 Web App deployment URL 到 source-of-truth fallback 邏輯到 Telegram supergroup 自動升級）
- **文件撰寫**：CLAUDE.md / README.md 都是 AI 寫的，包括 16 條 Common Pitfalls — 每條都是真的踩過的坑
