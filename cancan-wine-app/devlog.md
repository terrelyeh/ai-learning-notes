...
3. **酒款常換，配對要跟著更新**：自然酒常常售完、又不定期進新酒（菜單反而相對固定），每換一批酒，哪支酒配哪道菜、酒卡資訊都要跟著重做。
...
| **Menu** 菜單 | 主表 | 菜色（相對固定）：中英名、描述、價格、風味 | 透過 Pairings 連 Wines |
...
工具上線、店裡真的開始用之後，又陸續長出一些東西。這一段不是新功能炫技，而是「**把工具從 demo 打磨成日常真的好用**」的過程——而且每一塊都還是同一套協作模式。

### 8-1. 內容的溫度：讓 AI 產草稿、真人替換（Boggy 說 / Moa 說）

官方文案有了，但少了「人味」。一支酒，店員要推得有說服力；一道菜，客人會想知道主廚怎麼想。於是在酒款筆記加了「**Boggy 說**」、在菜色加了「**Moa 說**」。

做法是：AI 照 cancan 的**語氣準則**先產出**範例草稿**（26 則 Boggy、全 16 道菜的 Moa），真人之後在後台直接改成自己的版本。有意思的是「方向」是來回調出來的——一開始是平實的推薦，後來我說「結合一點個人經驗」（在義大利學食物設計時……），再後來「也可以結合經典電影」（《王牌冤家》《雲端情人》），AI 就負責把這些方向變成符合品牌語氣的草稿。

> **學到的事**：AI 最適合當「第一稿」。人味不該外包給 AI，但 AI 可以先把框架鋪好、把真人「從零開始」的門檻降到「改一改就好」。對忙碌的店來說，「改」永遠比「生」容易。

### 8-2. 一句話改變方案：把功能塞進「既有的格子」

我想做「**對廠商的銷售分析**」（哪家酒商的酒賣得好、毛利高、哪家在養滯銷庫存）。AI 第一版提案是新開一張「月底盤點表」。我隨口問了一句：「**還是用既有的庫存欄位？**」——方案立刻收斂成「複用店員本來就在維護的庫存數字 + 系統每月自動拍一張快照」，用『期初 − 期末 + 進貨』反推銷量。店員不用多學、多做一件事。

> **學到的事**：最好的功能設計常常是「少做」。使用者對自己流程的直覺，往往比 AI 的「正規解」更貼近現實——**AI 給結構，你給約束**，兩邊各擅勝場。

### 8-3. 從 demo 到日常：那些「不起眼但每天有感」的打磨

- **每次重開都要重打密碼** → 改成在裝置記住一段時間（預設 7 天、可在設定頁調），重開瀏覽器免再輸入。
- **內部頁面沒法傳給同事** → 每個分頁、每支酒、每道菜都給一個可分享的獨立網址，複製貼上就能分享，同事開了登入一次就直接到那頁。
- 其他順手的：配對加「**推薦理由**」（店員推薦有話可講）、酒單加「**年份排序**」、把缺的菜照補滿。

> **學到的事**：工具「能動」跟「好用」之間，差的常常就是這些小摩擦。上線不是終點——真正開始用之後，才看得到哪裡卡。
...
*整理自 AI 協作對話，2026-05-24（2026-05-25 增補第九章「上線之後」：Boggy 說／Moa 說、廠商分析設計取捨、持久登入與分享網址）*
